...
public interface RocketExporter {
     ...
     * @return new instance
     */
    static RocketExporter newInstance(Config config, PrintStream out) {
        return new Default(config, out);
    }
    ...
    class Default implements RocketExporter {
        ...
        Default(Config config, PrintStream out) {
            this.out = out;
            this.config = config;
        }
        ...
        private List<Message> exportMessages(String contextName, String id,
            ...
            List<RocketChatFileMessage> fileMessages = new ArrayList<>();

            while (true) {

                Response<RocketChatFileMessageWrapperDto> files = getService().getAllFileFromDirectMessages(authHeaders, id, limit, fileOffset).execute();
                if (files.body() != null && files.body().files != null) {
                    fileMessages.addAll(files.body().files);
                    fileOffset = limit + fileOffset;
                    if (fileOffset >= files.body().total)
                        break;
                }

            }
            this.out.printf("file messages for download size: %d chat: %s\n", fileMessages.size(), contextName);
            ...
            for (RocketChatFileMessage file : fileMessages) {
